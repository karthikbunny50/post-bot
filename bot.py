...
(
    ...
    CHANNEL_SELECTION, CONFIRM, CHANNEL_ACTION, CHANNEL_NAME, CHANNEL_ID
) = range(17)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Use /channel to manage channels."
    )

async def channel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Manage channels (add, remove, list)."""
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("You are not authorized to use this bot.")
        return
...
async def channel_action_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle channel management actions."""
    query = update.callback_query
    await query.answer()
    
    if query.data == "channel_add":
        await query.edit_message_text("Please enter the channel name (e.g., My Channel):")
        return CHANNEL_NAME
        ...
        for channel_name in CHANNELS.keys():
            keyboard.append([InlineKeyboardButton(channel_name, callback_data=f"remove_{channel_name}")])
        ...
        if channel_name in CHANNELS:
            ...
            os.environ['CHANNELS'] = json.dumps(CHANNELS)
            await query.edit_message_text(f"Channel '{channel_name}' removed successfully.")
        else:
            await query.edit_message_text("Channel not found.")
        return ConversationHandler.END

async def channel_name_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Store channel name and ask for channel ID."""
    context.user_data['channel_name'] = update.message.text
    await update.message.reply_text("Please enter the channel ID (e.g., @channel_username or -1001234567890):")
    return CHANNEL_ID

async def channel_id_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Store channel ID and update channels."""
    ...
    await update.message.reply_text(f"Channel '{channel_name}' added successfully with ID: {channel_id}")
    return ConversationHandler.END
...
async def post_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not CHANNELS:
        await update.message.reply_text("No channels configured. Use /channel to add channels first.")
        return ConversationHandler.END
...
async def channel_selection_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'file_id' in post_data[user_id]:
        await query.edit_message_text("Post preview:")
...
def format_post(data):
    ...
    if data.get('title_style') == 'quote':
        title_formatted = f"<blockquote>{data['title']}</blockquote>"
    else:
        title_formatted = f"<b>{data['title']}</b>"
    ...
    return (
        ...
        f"‣ Genre : {data['genre']}\n"
        f"╰━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"<blockquote>Synopsis: {data['synopsis']}</blockquote>\n\n"
        f"<blockquote>Powered By: {POWERED_BY_USERNAME}</blockquote>"
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle title style selection
    if query.data in ["normal_title", "quote_title"]:
        # This is handled in title_style_handler
        pass
...
def main():
    ...
    channel_conv = ConversationHandler(
        ...
        states={
            CHANNEL_ACTION: [CallbackQueryHandler(channel_action_handler)],
            CHANNEL_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, channel_name_input)],
            CHANNEL_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, channel_id_input)],
        },
        fallbacks=[],
        per_message=False  # Add this to suppress the warning
    )
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            IMAGE: [
                MessageHandler(filters.PHOTO, image_input),
